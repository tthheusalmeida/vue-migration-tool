  ...
  traverse(currentAst, {
    ImportDeclaration(path) {
      if (
        ...
        const isThereImportVueRouter = path.node?.specifiers.find(
          (spec) =>
            spec.local.name === "VueRouter" || spec.local.name === "Router"
        );
        if (isThereImportVueRouter) {
          ...
          const prop = t.importSpecifier(
            t.identifier(getModeType(modeProp)),
            t.identifier(getModeType(modeProp))
          );
          args.push(prop);
          ...
          stateManager.set(
            "importVueRouter",
            t.importDeclaration(args, t.stringLiteral("vue-router"))
          );
          path.remove();
        }
      }
    ...
    NewExpression(path) {
      if (
        ...
        const modeIndex = args[0].properties.findIndex(
          (prop) => prop.key.name === "mode"
        );
        if (modeIndex >= 0) {
          const baseIndex = args[0].properties.findIndex(
            (prop) => prop.key.name === "base"
          );
          const isThereBaseProp = baseIndex >= 0;
          const baseValue = isThereBaseProp
            ? args[0].properties[baseIndex].value
            : null;
          const identifier = stateManager.get("routerPropMode").value;
          args[0].properties[modeIndex] = t.objectProperty(
          ...
          properties.push(
            t.objectProperty(
              ...
              t.callExpression(t.identifier(getModeType(defaultMode)), [])
            )
          );
        }

        path.replaceWith(t.callExpression(t.identifier("createRouter"), args));
  ...
  traverse(currentAst, {
    Program(path) {
      ...
      Object.keys(store).forEach((item) => {
        if (item === "importVueRouter" && stateManager.get(item)) {
